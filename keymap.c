...
enum custom_keycodes {
    ...
    KC_TO_CLICKABLE_INC,
    KC_TO_CLICKABLE_DEC,
    KC_TO_CLICKABLE_TIME_INC, //　マウスレイヤを切り替えるまでの時間を調整するキーコードの定義 @miyazaki
...
uint16_t to_reset_time = 10000; // この秒数(千分の一秒)、CLICKABLE状態ならクリックレイヤーが無効になる。 For this number of seconds (milliseconds), the click layer is disabled if in CLICKABLE state.

const uint16_t click_layer = 1;   // マウス入力が可能になった際に有効になるレイヤー。Layers enabled when mouse input is enabled
...
const uint16_t ignore_disable_mouse_layer_keys[] = {KC_RGUI, KC_RCTL, KC_RALT, KC_RSFT, KC_MS_BTN2}; // 押してもマウスレイヤを解除しないキーコードのリスト @miyazaki
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case KC_TO_CLICKABLE_TIME_INC:
            if (record->event.pressed) {
                if(to_reset_time < 10000){
                    to_reset_time = to_reset_time + 1000; //マウスレイヤ解除するタイマの加算。1秒単位 MAX 10秒  @miyazaki
                }
            }
            return false;
        ...
        case KC_TO_CLICKABLE_TIME_DEC:
            if (record->event.pressed) {
                if(to_reset_time > 1000){
                    to_reset_time = to_reset_time - 1000; //マウスレイヤ解除するタイマの減算。1秒単位 最小1秒  @miyazaki
                }
            }
            return false;
         ...
         default:
            if  (record->event.pressed) {
                ...
                for (int i = 0; i < sizeof(ignore_disable_mouse_layer_keys) / sizeof(ignore_disable_mouse_layer_keys[0]); i++){
                    if (keycode == ignore_disable_mouse_layer_keys[i]){ // 上で定義したマウスレイヤを無効化しないキーの判定。引っかかったらマウスレイヤ継続 @miyazaki
                        enable_click_layer();
                        return true;
                    }
                }
    ...
    if (current_x != 0 || current_y != 0) {
        ...
        switch (state) {
            ...
            case SCROLLING:
            {
                ...
                current_h = rep_h / scroll_h_threshold * -1; // スクロール方向を反転させるためにマイナス化。戻す場合は-1を消す。@miyazaki
                current_v = -rep_v / scroll_v_threshold * -1; // 同上 @miyazaki
                current_x = 0;
                current_y = 0;
            }
                break;
...
// LED制御部 @miyazaki
// 下のサンプルはマウスレイヤの1だけパープルに点灯。それ以外は消灯。
// 0番目から60個分のLEDを操作するみたいな形式。
